...
<template>
  <div class="relative overflow-x-auto sm:rounded-lg m-4">
    <table class="w-full text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr
          ...
          v-for="item in itemsDisplay" :key="item.id" @click="item.check = !item.check">
          <td v-if="check" class="px-4 py-3">
            <input type="checkbox" v-model="item.check" />
          </td>
          <td class="px-4 py-4" v-for="column in columns" :key="column.key">
            <span v-if="column.color">
              <span class="rounded-full py-1 px-3" :style="{ backgroundColor: item[column.key] }"></span>
            </span>
            <span v-else-if="column.date">
              {{ fullDateFormat(item[column.key]) }}
            </span>
            <span v-else-if="column.status" class="flex justify-center">
              <span class="rounded-full text-white py-1 px-1"
                :class="[item[column.key] == 1 ? 'bg-green-600' : 'bg-red-600']">
                <v-icon :name="[item[column.key] == 1 ? 'fa-check' : 'fa-times']" /></span>
            </span>
            <span v-else-if="column.check" class="flex justify-center">
              <span class="rounded-full text-white py-1 px-1" :class="[item[column.key] ? 'bg-green-600' : 'bg-red-600']">
                <v-icon :name="[item[column.key] ? 'fa-check' : 'fa-times']" /></span>
            </span>
              ...
              <template v-slot:icon>
                <div>
                  <button type="button" class="flex text-sm rounded-full">
                    <v-icon name="co-options" class="w-6 h-6 rounded-full text-gray-600 dark:text-gray-200 p-1" />
                  </button>
                </div>
              </template>
              <div
                ...
                <ul class="py-1 text-left" role="none">
                  <li v-for="(option, index) in options" :key="index">
                    <span
                      ...
                      @click="action({ action: option.id, id: item.id })"><v-icon :name="option.icon" />{{ option.name
                      }}</span>
                  </li>
                </ul>
              </div>
    ...
    <nav class="flex items-center justify-between pt-4 pb-8" aria-label="Table navigation">
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Pagina
      ...
      <ul class="inline-flex -space-x-px text-sm h-8">
        <li class="mr-4 flex">
          <label class="flex items-center text-gray-500 dark:text-gray-400 px-3 h-8 ml-0" for="itemsPerPage">Filas por
            pagina</label>
