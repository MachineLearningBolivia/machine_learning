<script setup>
import { getCategoriasRequest, importCategoriasRequest } from "@/api/categoria";
...
async function action(action) {
  if (action.action === "update") {
    router.push({ path: "/update/categorias", query: { id: action.id } });
  }
}
...
async function handleSubmit(event) {
  ...
  try {
    await importCategoriasRequest(formData);
    items.value = [];
    loadData();
...
<template>
  <card-data title="Categorías" icon="fa-layer-group">
    <template v-slot:filters>
      ...
      <button-add to="/new/categorias"> Agregar Categoría </button-add>
    </template>
    <DataTable
      :items="itemsDisplay"
      :columns="columns"
